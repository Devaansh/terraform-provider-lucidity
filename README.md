...
- `docs/qa/` - Complete QA testplan, results and logs (access tokens and accounts already deleted, don't bother wasting your time)
...
### Bringing in an Azure/GCP account (or an AWS account onboarded outside Terraform)

Onboarding a brand-new resource is AWS-only today; every Azure/GCP tenant —
and any AWS tenant that already exists on Lucidity — enters Terraform via
import instead:
...
Import ID is `<cloud_provider>/<cloud_provider_account_id>`. Two fields
can't be recovered by import (`aws_iam_external_id`, `lucidity_product_list`
— neither is ever returned by Lucidity's list API) — write a resource block
with the real values matching that account, or the next apply forces a
destroy+recreate.
...
Full test evidence: [docs/qa/account-management.md](docs/qa/account-management.md).
